...
def test_mlp(group: equivit.geometry.Group, activation):

    # Let's be lenient with large groups
    ERROR_TOL = 4e-6 if group.order() < 20 else 4e-5
    ...
    dims_in = [torch.randint(1, 24, (1,), generator=rng).item() for _ in irreps]
    dims_out = [torch.randint(1, 24, (1,), generator=rng).item() for _ in irreps]
    ...
    mlp = equivit.nn.EquivariantMLP(group, 
                                       dims_in=dims_in,
                                       homogeneous_space_copies=num_homog_copies,
                                       dims_out=dims_out,
                                       activation=activation)

    for _ in range(4): # test 4 random inputs
        N = torch.randint(32, 96, (1,), generator=rng).item()
        x = equivit.random_irrep_tensors(group, shape=(N,), dims=dims_in, rng=rng)
        y = mlp(x)
